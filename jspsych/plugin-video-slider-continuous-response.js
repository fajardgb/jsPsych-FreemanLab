...
var jsPsychVideoSliderContinuousResponse = (function (jspsych) {
    ...
    const info = {
        name: "video-slider-continuous-response",
        parameters: {
    ...
    /**
     * **video-slider-continuous-response**
     *
     * jsPsych plugin for playing a video file and getting a continuous slider response
     *
     * @author Josh de Leeuw
     * @modifier Yara Studer
     */
    class VideoSliderContinuousResponsePlugin {
        constructor(jsPsych) {
      ...
      `);
            ...
            // setup stimulus
            var video_html = '<video id="jspsych-video-slider-continuous-response-stimulus-video"';
            if (trial.width) {
            ...
            if (!video_preload_blob) {
                for (var i = 0; i < trial.stimulus.length; i++) {
                    ...
                    if (type == "mov") {
                        console.warn("Warning: video-slider-continuous-response plugin does not reliably support .mov files.");
                    }
                    video_html += '<source src="' + file_name + '" type="video/' + type + '">';
                }
            }
            video_html += "</video>";
            var html = '<div id="jspsych-video-slider-continuous-response-wrapper" style="margin: 100px 0px;">';
            html += '<div id="jspsych-video-slider-continuous-response-stimulus">' + video_html + "</div>";
            html +=
                '<div class="jspsych-video-slider-continuous-response-container" style="position:relative; margin: 0 auto 3em auto; width:';
            ...
            html +=
                ...
                '" step="' +
                trial.step +
                '" id="jspsych-video-slider-continuous-response-response" oninput=registerResponse(this.value)';
            ...
            if (trial.require_movement) {
                const enable_button = () => {
                    display_element.querySelector("#jspsych-video-slider-continuous-response-next").disabled = false;
                };
                display_element
            ...
            const end_trial = () => {
                ...
                display_element
                    .querySelector("#jspsych-video-slider-continuous-response-stimulus-video")
                    .pause();
                display_element.querySelector("#jspsych-video-slider-continuous-response-stimulus-video").onended = () => { };
            ...
            function enable_slider() {
                ...
                if (!trial.require_movement) {
                    document.querySelector("#jspsych-video-slider-continuous-response-next").disabled = false;
                }
            }
        ...
        simulate_visual(trial, simulation_options, load_callback) {
            ...
            load_callback();
            const video_element = display_element.querySelector("#jspsych-video-button-continuous-response-stimulus");
            const respond = () => {
                if (data.rt !== null) {
    ...
    VideoSliderContinuousResponsePlugin.info = info;

    return VideoSliderContinuousResponsePlugin;
